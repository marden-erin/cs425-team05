...
*/
  async updateClusterInformation(clusterName: string, userName: string, newClusterName:string, visibility: boolean) {
    const input = {
      clusterName,
      userName,
      newClusterName,
  ...
  async getBookFromCluster(clusterName: string, userName: string, title:string, pageNumbers: number, bookCoverLink: string)
  {
    const res = await fetch(`/api/book?clusterName=${clusterName}&userName=${userName}&title=${title}&pageNumbers=${pageNumbers}&bookCoverLink=${bookCoverLink}`);
    const data = await res.json();
    ...
    async addBookToCluster(clusterName: string, userName: string, book: Book) {
      ...
      return data;
        ...
        // Returns success or failure
        async deleteBookFromCluster(clusterName: string, userName: string, book: Book) {
          const { title, pageCount, cover } = book;
